...
class SubSpecLossySDGeneratorBase(_SubSpecSDGeneratorBase):
    ...
    Drafting is unchanged (still produces an N-step, K-wide top-K cumulative tree).
    Verification differs:
      - accept if match target token OR target prob >= threshold
      - then gate acceptance by a bottom-up window criterion
    """
